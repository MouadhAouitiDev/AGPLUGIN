...
class AGPLUGIN{
	...
	//static can be with all the type of function public or protected or private we can calling static methode or attribut with out instancier the class 
	 ...
	 function custom_post_type(){
		 register_post_type('MAGASIN',['public'=> true,'label'=>'MAGASIN']);
 ...
 if ( class_exists( 'AGPLUGIN' )){
	
	 $AGPLUGIN = new AGPLUGIN();

 }
 
 //activation 
 require_once plugin_dir_path(__FILE__).'inc/agplugin-activate.php';
 register_activation_hook(__FILE__,array( 'AgpluginActivate', 'activate' )); 
 
 
 //desactivation 
 require_once plugin_dir_path(__FILE__).'inc/agplugin-deactivate.php';
  register_deactivation_hook(__FILE__,array('AgpluginDeactivate' , 'desactivate' ));
